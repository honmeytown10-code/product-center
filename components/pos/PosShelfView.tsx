...
import { Search, ChevronDown, Check, ChevronLeft, ChevronRight, List, RefreshCw, Link2Off, CheckCircle2, Circle, Filter, LayoutGrid, Layers, Menu, Image, AlignJustify } from 'lucide-react';
  ...
  return (
    <div className="flex h-full w-full bg-[#F5F6FA] overflow-hidden font-sans relative">
      ...
      <div className="w-[320px] bg-white border-r border-gray-200 flex flex-col z-10 shrink-0 shadow-[2px_0_10px_rgba(0,0,0,0.02)]">
          ...
          <div className="flex-1 overflow-y-auto">
              {offShelfList.length === 0 ? (
                  ...
                  offShelfList.map((item) => {
                      ...
                      return (
                          <div key={item.id} className="flex flex-col px-5 py-4 border-b border-gray-50 hover:bg-[#00C06B]/5 transition-colors group">
                              <div className="flex items-center justify-between mb-2">
                                  <span className="font-bold text-gray-700 text-sm truncate pr-4 flex-1">{item.name}</span>
                                  <button 
                                      onClick={() => setShelfManagementItem(item)} 
          ...
          <div className="h-12 border-t border-gray-100 flex items-center justify-end px-4 text-xs text-gray-500 shrink-0 bg-white">
              <span className="mr-3">{`共 ${offShelfList.length} 条`}</span>
              <span className="mr-2">1/1页</span>
              <div className="flex space-x-1">
                  <button className="w-6 h-6 border rounded flex items-center justify-center bg-gray-50 text-gray-300 cursor-not-allowed"><ChevronLeft size={12}/></button>
                  <button className="w-6 h-6 border rounded flex items-center justify-center bg-gray-50 text-gray-300 cursor-not-allowed"><ChevronRight size={12}/></button>
              </div>
          </div>
      </div>
