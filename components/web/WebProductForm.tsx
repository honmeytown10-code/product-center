...
export const WebProductForm: React.FC<WebProductFormProps> = ({
    ...
    const [comboGroupCards, setComboGroupCards] = useState<ComboGroupCard[]>(() => (
        mode === 'create'
            ...
            : [
                ...
                { id: 'combo-optional-1', type: 'optional', title: '可选分组', desc: '已添加 1 个可选分组' },
            ]
    ));
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <div className="flex items-center gap-8">
                            {[
                                ...
                                <label
                                    key={option.key}
                                    className={`flex items-center gap-2 text-sm ${(isChannelForm || (isWeightProduct && option.key === 'multi')) ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}
                                >
                                    <input
                                        type="radio"
                                                    ...
                                                    <table className="w-full border-collapse table-fixed">
                                                        <thead className="bg-[#F7F8FA]">
                                                            <tr className="text-left text-xs font-bold text-gray-500">
                                                                ...
                                                                {!isChannelForm && showAddonLimit && <th className="w-[88px] px-3 py-3 border-b border-gray-200">限购</th>}
                                                                {!isChannelForm && showAddonPrice && <th className="w-[88px] px-3 py-3 border-b border-gray-200">初始价格</th>}
                                                                {!isChannelForm && showAddonSpecPrice && <th className="w-[92px] px-3 py-3 border-b border-gray-200">规格加价</th>}
                                                                {showAddonStatus && <th className="w-[120px] px-3 py-3 border-b border-gray-200">{isChannelForm ? '渠道状态' : '商品状态'}</th>}
                                                                {!isChannelForm && <th className="w-[64px] px-3 py-3 border-b border-gray-200">操作</th>}
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            {addonConfigRows.filter(row => row.groupName === groupName).map(row => (
                                                                <tr key={row.id} className="align-top text-[13px] text-[#1F2129]">
                                                                    ...
                                                                    {!isChannelForm && showAddonLimit && <td className="px-3 py-3 border-b border-gray-100">
                                                                    ...
                                                                    {!isChannelForm && showAddonPrice && <td className="px-3 py-3 border-b border-gray-100">
                                                                    ...
                                                                    {!isChannelForm && showAddonSpecPrice && <td className="px-3 py-3 border-b border-gray-100">
                                                                        <button type="button" disabled title="规格加价编辑器尚未接入当前原型" className="cursor-not-allowed text-[13px] font-bold text-gray-400">
                                                                    ...
                                                                    {showAddonStatus && <td className="px-3 py-3 border-b border-gray-100">
                                                                        {isChannelForm ? (
                                                                            <div className="flex items-center gap-2">
                                                                                <Switch
                                                                                    active={row.addonStatus === 'on'}
                                                                                    onClick={() => updateAddonRow(row.id, 'addonStatus', row.addonStatus === 'on' ? 'off' : 'on')}
                                                                                />
                                                                                <span className={row.addonStatus === 'on' ? 'text-[#008F4C]' : 'text-gray-400'}>
                                                                                    {row.addonStatus === 'on' ? '已启用' : '已禁用'}
                                                                                </span>
                                                                            </div>
                                                                        ) : (
        ...
        const renderCompletedPanel = () => (
            <div className="flex min-h-[520px] items-center justify-center rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
                <div className="text-center">
                    ...
                    <div className="mt-6 text-[32px] font-black text-[#1F2129]">
                        {task === 'sync'
                            ? '同步成功，商品已下发门店'
                            : successMode === 'edit'
                                ? '更新模板任务已提交'
    ...
    return (
        <div className="flex h-full w-full min-w-0 flex-col overflow-hidden bg-[#F5F6FA] animate-in slide-in-from-right duration-300">
            {/* Header */}
            <div className="h-16 px-5 bg-white border-b border-[#E8E8E8] flex min-w-0 justify-between items-center shrink-0 z-20">
