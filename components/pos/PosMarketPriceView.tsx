...
import { BadgeDollarSign, Clock3, Delete, Image, Minus, Plus, RotateCcw } from 'lucide-react';
...
const INITIAL_ITEMS: MarketPriceItem[] = [
  ...
  { id: 'mp3', name: '蓝鳍金枪鱼刺身', category: '海鲜河鲜', unit: '份', standardPrice: 168, price: 168, updatedAt: '使用标准价' },
  ...
  { id: 'mp11', name: '春笋', category: '时令蔬菜', unit: '份', standardPrice: 38, price: 42, updatedAt: '今天 09:06' },
  ...
  { id: 'mp18', name: '法式羊排', category: '肉禽', unit: '份', standardPrice: 118, price: 118, updatedAt: '使用标准价' },
...
function MarketPriceDialog({ item, onClose, onConfirm }: { item: MarketPriceItem; onClose: () => void; onConfirm: (price: number) => void }) {
  ...
  const adjust = (amount: number) => {
    ...
    setValue(formatPrice(Math.min(99999.99, Math.max(0.01, current + amount))));
  };
  return <PosDialog title="修改时价" className="pos-market-price-dialog" onClose={onClose} footer={<>
    ...
    <div className="pos-market-editor">
      <section className="pos-market-editor-main">
        <div className="pos-market-product-head"><div><strong>{item.name}</strong><span>单规格标品 · 当前门店 POS</span></div><em>时价菜</em></div>
        <div className="pos-market-price-compare">
          <div><span>标准价格</span><strong>¥{formatPrice(item.standardPrice)}<small> / {item.unit}</small></strong></div>
          <div><span>当前时价</span><strong>¥{formatPrice(item.price)}<small> / {item.unit}</small></strong></div>
        </div>
        <div className="pos-market-new-price" data-invalid={!!value && !valid}>
          <span>新时价</span><div><b>¥</b><strong>{value || '0'}</strong><small>/ {item.unit}</small></div>
        </div>
        {!valid && <p className="pos-market-error">请输入大于 0 且不超过 99999.99 的价格，最多保留两位小数。</p>}
        <div className="pos-market-adjust" aria-label="快捷调价">
          <button type="button" onClick={() => adjust(-5)}><Minus size={15} />5</button>
          <button type="button" onClick={() => adjust(-1)}><Minus size={15} />1</button>
          <button type="button" onClick={() => adjust(1)}><Plus size={15} />1</button>
          <button type="button" onClick={() => adjust(5)}><Plus size={15} />5</button>
        </div>
...
export const PosMarketPriceView: React.FC<{ showImage: boolean; search: string; onReset: () => void }> = ({ showImage, search, onReset }) => {
  ...
  const save = (price: number) => {
    ...
    setResult(`${editing.name} 时价已更新为 ¥${formatPrice(price)} / ${editing.unit}`);
    setEditing(null);
  };
  return <div className="pos-view">
    ...
    <div className="pos-grid-scroll"><div className="pos-grid pos-market-grid">{visible.map(item => {
      ...
      return <button key={item.id} className="pos-card pos-market-card" onClick={() => setEditing(item)} aria-label={`${item.name}，当前时价 ${item.price} 元每${item.unit}`}>
        ...
        <div className="pos-market-meta"><span><BadgeDollarSign size={13} />时价菜</span><span>单规格标品</span></div>
        <div className="pos-market-reference">标准价 ¥{formatPrice(item.standardPrice)} / {item.unit}</div>
        <div className="pos-market-card-footer"><div><small>当前时价</small><strong>¥{formatPrice(item.price)}<em>/{item.unit}</em></strong></div><span><Clock3 size={12} />{item.updatedAt}</span></div>
      </button>;
    })}</div>
